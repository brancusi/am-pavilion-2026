(ns amp.views.screening-view
  (:require ["gsap" :refer [gsap]]
            [amp.components.elements.video-player :refer [VideoPlayer]]
    ...
    (d/div {:class "flex flex-col w-screen min-h-screen h-full bg-zinc-100 border-red-500"
           ...
           (d/div {:class "flex min-w-full h-full items-center justify-center p-4 grow"}
                  ...
                  (d/div {:class "w-full sm:w-3/4"}
                         (d/p {:class "text-xl lg:text-3xl font-mono"}
                              title)
                         (d/p {:class "text-md lg:text-xl font-mono mb-4"}
                              (str date))
                         (d/div {:class "my-4"}
                                ($ VideoPlayer {:playback-id current-route-production-id}))
                         (d/div {:class "flex flex-col text-md lg:text-xl font-mono"}
                                (when shot-by (d/p {:class "text-md lg:text-xl font-mono"}
                                                   (str "Shot by: " shot-by)))
                                (when contact
                                  (d/div
                                   (d/span {:class "text-md lg:text-xl font-mono"}
                                           "Contact: ")
                                   (d/a {:class "text-md lg:text-xl font-mono"
