(ns amp.components.sections.form-labs-section
  (:require
   ...
   [amp.components.elements.expandable-text-area :refer [expandable-text-area]]
...
(defnc header
  ...
  (d/div {:class ""}
         ...
         (d/div {:class "px-4 mt-12 lg:mt-16 max-w-4xl"}
                (d/h1 {:class "font-helvetica font-bold text-3xl lg:text-4xl text-white mb-6"}
                      "Presentation for Form Labs collaboration and sponsorship"))

         (d/div {:class "w-1/2 lg:w-1/4 lg:max-w-64 mt-4 lg:mt-8 px-4"}
                (d/img {:src "images/icons/formlabs.png"}))
...
(defnc press-release
  []
  ($ expandable-text-area {:section-hint "Overview"
                           :title "Press release"
                           :expand-button-label "Read more"
                           :preview-text (d/p {:class "font-medium text-xl"}
                           ...
                           :full-text (d/p "full text")
                           :footer-text (d/p "Footer")})

  #_(let [[expanded? set-expanded] (hooks/use-state false)]
      ($ section-block
         {:idx 1
          :eyebrow "NEWS"
          :title "PRESS RELEASE"}
...
(defnc form-labs-section
  []
  ($ section
     {:key "form-labs-section"
      :section-id "form-labs-section"}
